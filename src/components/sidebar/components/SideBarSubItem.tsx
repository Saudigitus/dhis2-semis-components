...
export default function SideBarSubItem(props: SideBarSubItemProps) {
    const { icon, label, badgeInfo, disabled, appUrl, active, action, title } = props

    return (
        <div data-test={"menu-" + label.toLocaleLowerCase() + "-" + title.toLocaleLowerCase() + "-sidebar"} onClick={action} className={style.subItemLink}>

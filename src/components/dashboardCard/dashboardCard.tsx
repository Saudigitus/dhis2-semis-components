...
const DashboardCard = ({ icon, actions, alignActions, size, contents, dataTest }: CardDashboardProps) => {
  ...
  return (
    <Box data-test={dataTest} className={classNames(style.dinamicBox, style[getCardSize()])}>
      <Card className={classNames(style.cardContainer)}>
        ...
        <CardActions dataTest={dataTest} actions={actions} align={alignActions ?? "end"} />
      </Card>
    </Box>
  )
}
